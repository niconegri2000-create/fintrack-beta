...
function Calendar({ className, classNames, showOutsideDays = true, month: monthProp, onMonthChange, ...props }: CalendarProps) {
  ...
  const weekStartsOn = prefs.firstDayOfWeek === "monday" ? 1 : 0;

  const [internalMonth, setInternalMonth] = useState<Date>(monthProp ?? props.defaultMonth ?? props.selected as Date ?? new Date());
